...
const expand = (s, len) =>
    spliceString(
        ...
        s.lastIndexOf(" "),
        0,
        " ".repeat(Math.max(0, len - s.length))
    )
const getStatsLine = (it, index) => {
    ...
    return `<b>${index + 1}.</b> @${it.username}${secret}\t\t<b>${showValue(
        it.value
    )}</b>\t\t=\t\t<i>+${it.plus} / -${it.minus}</i>`
}
...
module.exports = bot => msg => {
    ...
    if (msg.limit == 6) {
        bot.sendMessage(msg.chatId, "Некорректный username!", {
            parse_mode: "HTML",
        })
        return
    }
    if (msg.type == "stats") {
        bot.sendMessage(msg.chatId, getStatsMessage(msg.result, msg.page), {
            parse_mode: "HTML",
        })
        return
    }
    bot.sendMessage(msg.chatId, getReputationInfo(msg.reputation), {
        parse_mode: "HTML",
    })
}
